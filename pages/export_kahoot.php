...
<div id="page-wrapper">
	...
	<h3 style="text-align:center;">Seleziona argomenti da esportare</h3>
	<div style="text-align:center;width:100%">
		<a class="btn btn-success" target="_blank" href="esporta_kahoot.php?id_utente=<?php echo $id_utente; ?>">Crea Excel con gli argomenti selezionati</a></div>
		<?php
				...
				<table class="table">
					...
					<tbody>
						...
						while($row2=($dataset2->fetch_assoc())) {
							...
							$dataset3=eseguiQuery("select count(*) as tot from ct_argomenti_kahoot where fk_utente=$id_utente and fk_argomento = $row2[id_argomento]");
							$row3 = $dataset3->fetch_assoc();
						
							if($eo%2==0) $classe="even";
							else $classe="odd";
							
							if($row3["tot"]>0) $classe="green";
							echo "<tr class='$classe'>";
							echo "<td>$row2[nome_argomento]</td>\n";
							if($row3["tot"]==0) {
								echo "<td style='text-align:right;'><button class='btn btn-info' onclick='aggiungi_argomento_kahoot($row2[id_argomento],$id_utente)'>Aggiungi</button></td>\n";
							}else {
								echo "<td style='text-align:right;'><button class='btn btn-danger' onclick='rimuovi_argomento_kahoot($row2[id_argomento],$id_utente)'>Rimuovi</button></td>\n";
							}
							echo "</tr>";
							$eo++;
